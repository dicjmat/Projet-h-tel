Public Class iListeCOmmande
    ...
    Private Sub txtRComm_TextChanged(sender As Object, e As TextChangedEventArgs) Handles txtRComm.TextChanged
        Dim res = From Co In bd.tblCommande Join Em In bd.tblEmploye On Co.noEmpl Equals Em.noEmpl
                  Where _noEmpl = Em.noEmpl And ((Em.nomEmpl + " " + Em.prenEmpl).StartsWith(txtRComm.Text) Or (Em.prenEmpl + " " + Em.nomEmpl).StartsWith(txtRComm.Text) Or Co.dateCommande.ToString.StartsWith(txtRComm.Text) Or Co.etatCommande.StartsWith(txtRComm.Text))
                  Select Co
    End Sub
End Class
